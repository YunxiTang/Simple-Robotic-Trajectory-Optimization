...
%% al ms ddp
x_almsddp = load('x_almsddp.mat');
u_almsddp = load('u_almsddp.mat');

x_al = x_almsddp.xsol;
u_al = u_almsddp.usol;
%% rlb ms ddp
x_rlbmsddp = load('x_rlbmsddp.mat');
u_rlbmsddp = load('u_rlbmsddp.mat');
...
h2=plot(0.0, pi, 'gx', 'LineWidth', 5.0); hold on;
...
ylabel('$\theta[rad]$', 'Interpreter', 'latex', 'FontSize', 18, 'FontAngle','italic');
title('$CartPole$', 'Interpreter', 'latex', 'FontSize', 20, 'FontAngle','italic');
ld2 = legend([h1,h2,h3,h4,h5,h6,h7],'$\textbf{x}_0$','$\textbf{x}_f$','$AL-DDP$', '$Ours$','$SS$','$DIRCOL$', '$SRLB$',...
