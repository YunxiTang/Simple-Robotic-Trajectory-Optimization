...
%%% form the constraints as a whole function
constraint_func = @(x,u)([ x(4)-pi/2;
                          -x(4)-pi/2;
                           x(5)-pi/2;
                          -x(5)-pi/2]); % <= 0
...
%%% Function Setup %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Setup_Functions_barrier(params, fallcat, cost,constraint_func);
...
cons = constraint();
tstart = tic;
[xsol, usol, Ksol] = solver.Solve(fallcat,cost,cons,params);
telapsed = toc(tstart)
