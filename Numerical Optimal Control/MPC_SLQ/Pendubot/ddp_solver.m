...
function [SOL,K_f,x_rep] = ddp_solver(x_now,x_ref,u_ref,n_h,model,cost,K_lqr,...
...
%%%        stop_criterion: stop criterion          [1]
    ...
    u{n_h} = zeros(numel(u_max),1);
    K{n_h} = zeros(numel(x_now),1);
    for i=1:n_h-1
        u{i} = u_ref(:,i) + K_lqr*((x_ref(:,i) - x_new{i}));
    ...
    for i=1:num_iter
        ...
        for k=1:n_h-1
            J(i) = J(i) + cost.L(x{k}, u{k}, x_ref(:,k), u_ref(:,k), dt);
        end
