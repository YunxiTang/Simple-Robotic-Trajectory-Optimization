...
%         u{i} = 0.5 * u_max * cos(0.2 * i * dt);
    ...
    for j=1:N-1
        % RK45 Integration
        k1 = dyn.Dynamics(0, x_new{j},             u{j});
        k2 = dyn.Dynamics(0, x_new{j} + 0.5*dt*k1, u{j});
        k3 = dyn.Dynamics(0, x_new{j} + 0.5*dt*k2, u{j});
        k4 = dyn.Dynamics(0, x_new{j} +     dt*k3, u{j});
...
%                 end
                ...
                k2 = dyn.Dynamics(0, x_new{k} + 0.5*dt*k1, u{k});
                k3 = dyn.Dynamics(0, x_new{k} + 0.5*dt*k2, u{k});
                k4 = dyn.Dynamics(0, x_new{k} +     dt*k3, u{k});
        ...
        for k=1:N-1
            J(i) = J(i) + cost.L(x{k}, u{k}, x_star, dt);
        end
        ...
        for k=N-1:-1:1
            Q_x{k} = cost.L_x(x{k}, u{k}, x_star, dt) + ...
                     dyn.Phi(x{k},u{k}, dt).' * V_x{k+1};
            Q_u{k} = cost.L_u(x{k}, u{k}, x_star, dt) + ...
                     dyn.beta(x{k}, u{k}, dt).' * V_x{k+1};
            Q_xx{k} = cost.L_xx(x{k}, u{k}, x_star, dt) ...
                      + dyn.Phi(x{k}, u{k}, dt).' * V_xx{k+1} ...
                      * dyn.Phi(x{k}, u{k}, dt);
            Q_uu{k} = cost.L_uu(x{k}, u{k}, x_star, dt) ...
                      + dyn.beta(x{k}, u{k}, dt).' * V_xx{k+1} ...
                      * dyn.beta(x{k}, u{k}, dt);
            Q_xu{k} = cost.L_xu(x{k}, u{k}, x_star, dt) ...
                      + dyn.Phi(x{k}, u{k}, dt).' * V_xx{k+1} ...
                      * dyn.beta(x{k}, u{k}, dt);
            Q_ux{k} = cost.L_ux(x{k}, u{k}, x_star, dt) ...
                      + dyn.beta(x{k}, u{k}, dt).' * V_xx{k+1} ...
                      * dyn.Phi(x{k}, u{k}, dt);
