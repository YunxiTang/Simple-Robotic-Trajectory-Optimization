...
params.umax =  [4.5;4.5];
params.umin = -[4.5;4.5];
params.xub  = [ 5.0; 5.0; 5.0; 5.0];
params.xlb  = [-5.0; -5.0;-5.0;-5.0];
params.nx = numel(params.xub);
...
for k=0:(params.N-1)
    ...
    gk = [0.5*0.5 - ((Xk(1)-0.0)*(Xk(1)-0.0) + (Xk(2)-1.0)*(Xk(2)-1.0));
          0.6*0.6 - ((Xk(1)-1.3)*(Xk(1)-1.3) + (Xk(2)-1.3)*(Xk(2)-1.3));
          0.4*0.4 - ((Xk(1)-2.0)*(Xk(1)-2.0) + (Xk(2)-2.5)*(Xk(2)-2.5))];
    g = {g{:}, gk};
    lbg = [lbg; 
           [-inf;-inf;-inf]];
    ubg = [ubg; 
