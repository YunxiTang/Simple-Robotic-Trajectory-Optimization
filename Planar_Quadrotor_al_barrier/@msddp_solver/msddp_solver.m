    ...
    methods
        ...
        function [] = solver_Callback(obj,xbar,ubar,params)
            ...
            for i = 1:obj.M
                clr = [i, 0.5 * i, 0.5 * i] / obj.M;
                figure(222);
                plot(params.t{i}(1:end-1),ubar{i}(:,1:end-1),'Color',clr,'LineWidth',2.0);hold on; 
            end
...
%                     x0 = x{i-1}(:,end) -  new_dft(:,i-1);
        ...
        function [V,x,u] = ForwardIteration(obj,xbar,ubar,Vprev,du,K,DV,rbt,cst,path_constraint,final_constraint,params)
            ...
            while alpha > 1e-5
                ...
                ratio = (V - Vprev)/(dV);
                if params.Debug == 1 && mod(obj.iter,5)==0
