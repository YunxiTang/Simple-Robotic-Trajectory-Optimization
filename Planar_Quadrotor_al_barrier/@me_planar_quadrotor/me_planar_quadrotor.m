classdef me_planar_quadrotor < handle
    %Car A 2D Car model
    ...
    methods
        function obj = me_planar_quadrotor()
...
%                 clf(fig);
                ...
                obj.draw_circle(xi,yi);
                obj.draw_arrow(xi,yi,thetai);
                xlim([0,10]);
                axis equal;
                grid on;
                pause(0.05);
        ...
        function [] = draw_rotor(obj, x, y, theta)
            ...
            scatter(rotor_L_x, rotor_L_y,'LineWidth',2.0,'MarkerFaceColor','b','MarkerEdgeColor','b',...
                    'MarkerFaceAlpha',0.5); 
            hold on;
            scatter(rotor_R_x, rotor_R_y,'LineWidth',2.0,'MarkerFaceColor','b','MarkerEdgeColor','b',...
                    'MarkerFaceAlpha',0.9); 
            hold on;
        end
        
        function [] = draw_circle(obj,x,y)
            xc = x;
            yc = y;
            d = obj.l / 2;
            ds = 0:0.1:2*pi;
        ...
        function [] = draw_arrow(obj,x,y,theta)
            xc = x;
            yc = y;
            len = obj.l / 2; 
            phi = theta + pi/2;
