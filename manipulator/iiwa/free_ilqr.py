...
class free_iLQR():
    ...
    def __init__(self, system, num_timesteps, 
            input_port_index=0, delta=1e-6, beta=0.9, gamma=0.0):
        """
        Args:
        ...
        kp = np.diag( [150,150,150,150,50,20, 5] )
        kd = np.diag( [ 50, 50, 50, 50, 5, 5, 1] )
        self._K = np.hstack( (kp, kd) )
        self.K = np.repeat( -self._K[:,:,None], self.N-1, axis=2 )
        # print(self.K.shape)
    ...
    def init_forward(self):
        """
            Initial Forward Pass With Proper Initial Controller
        """
        L = 0
        ...
        kp = np.diag( [150,150,150,150,50,20, 5] )
        kd = np.diag( [ 50, 50, 50, 50, 5, 5, 1] )
        ref_pos = self.x_nom.copy()
        x[:,0] = self.x0
        for t in range(0,self.N-1):
            self.system.SetPositionsAndVelocities(self.context, x[:, t])
            tau_g = -self.system.CalcGravityGeneralizedForces(self.context)
            S = self.system.MakeActuationMatrix().T
            u_ff = tau_g
            u_fb = kp @ (ref_pos - x[0:7, t]) + kd @ (0. - x[7:14, t])
            u[:,t] = S @ ( u_ff + u_fb  )
                
            try:
                x[:,t+1] = self._calc_dynamics(x[:,t], u[:,t])
            except RuntimeError as e:
                # If simulation is unstable, set loss be inf and stop simulation.
                print("Warning: Unstable Simulation in Linesearch")
                L = np.inf
                break
